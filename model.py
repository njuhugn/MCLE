...
class GPT2LMHeadModel(GPT2PreTrainedModel):
    ...
    def __init__(self, config):
        ...
        self.image_input_proj = nn.Linear(config.n_embd, config.n_embd)
        self.image_output_proj = nn.Linear(config.n_embd, config.n_embd)
    ...
    def forward(
        ...
        if labels is not None:
            ...
            # semanticCL
            img_ques_cap_embeddings = torch.cat(
                [torch.mean(torch.cat([image_input[i], text_input[i, :cap_end[i], :]], dim=0), dim=0).unsqueeze(0) for i
                 in
                 range(0, batch_size)])
            ans_embeddings = torch.cat(
                [torch.mean(text_output[i, cap_end[i] - 1: cap_end[i], :], dim=0).unsqueeze(0) for i in
                 range(0, batch_size)])
            img_ques_cap_proj = F.normalize(self.semantic_input_proj(img_ques_cap_embeddings), dim=-1)
            ans_proj = F.normalize(self.semantic_output_proj(ans_embeddings), dim=-1)
            loss_sem = self.cl_loss(img_ques_cap_proj, ans_proj)
            # imageCL
            img_ques_embeddings = torch.cat(
                [torch.mean(torch.cat([image_input[i], text_input[i, :ques_end[i], :]], dim=0), dim=0).unsqueeze(0) for
                 i in
                 range(0, batch_size)])
            cap_ans_embeddings = torch.cat(
                [torch.mean(text_output[i, ques_end[i] - 1: cap_end[i] - 1, :], dim=0).unsqueeze(0) for i in
                 range(0, batch_size)])
            img_ques_proj = F.normalize(self.image_input_proj(img_ques_embeddings), dim=-1)
            cap_ans_proj = F.normalize(self.image_output_proj(cap_ans_embeddings), dim=-1)
            # hard_sample

            ques_embeddings = torch.cat(
                [torch.mean(text_input[i, :ques_end[i], :], dim=0).unsqueeze(0) for i in range(0, batch_size)])
            ques_proj = F.normalize(ques_embeddings, dim=-1)
            ans_embeddings = torch.cat(
                [torch.mean(text_input[i, cap_end[i]: cap_end[i] + 1, :], dim=0).unsqueeze(0) for i in
                 range(0, batch_size)])
            ans_proj = F.normalize(ans_embeddings, dim=-1)
            sim_q = ques_proj @ ques_proj.T
            sim_a = ans_proj @ ans_proj.T
            score = torch.exp(sim_q - sim_a)
            loss_img = self.cl_loss(img_ques_proj, cap_ans_proj, score)
            # instance_CL
            loss_ins = 0
            if is_cam != 0:
                pred = torch.cat([shift_logits[i, cap_end[i] - 1, :].unsqueeze(0) for i in range(0, batch_size)])
                # a = torch.cat([shift_labels[i, cap_end[i]-1].unsqueeze(0) for i in range(0, batch_size)])
                ...
                w_ind = word_grad_cam_sigmoid.sort(1, descending=True)[1][:, :2]
                ...
                v_ind = visual_grad_cam.sort(1, descending=True)[1][:, :2]
            ...
            loss += self.alpha * loss_sem + self.beta * loss_img + self.gama * loss_ins
        if not return_dict:
...
class CELoss(nn.Module):
    ...
    def forward(self, pred, target, hard_negative_weight=None):
        ...
        if hard_negative_weight is not None:
            loss = -1. * pred.gather(1, target.unsqueeze(-1)) + torch.log(
                torch.exp(pred * hard_negative_weight + eps).sum(dim=1))
        else:
            loss = -1. * pred.gather(1, target.unsqueeze(-1)) + torch.log(torch.exp(pred + eps).sum(dim=1))
        # loss = -1. * pred.gather(1, target.unsqueeze(-1)) + torch.log(torch.exp(pred + eps) * hard_negative_weight.sum(dim=1))
        return loss.mean()
